...
//import service.Wirelessmeshpservice.*;
import service.*;


/** Presence service is a CRDT entity that uses Vote to manage user online presence. */
@CrdtEntity
public class PresenceDeviceEntity {
  private static final Logger logger = LogManager.getLogger(PresenceDeviceEntity.class);

  private Vote presence = null;
  private int users = 0;

  public PresenceDeviceEntity(Vote presence, Context ctx) {
   ...
   * it.
   */
  @CommandHandler OnlineStatus monitor(PresenceDevice user, StreamedCommandContext<OnlineStatus> ctx) {
